...
- `opencode/AGENTIC-GUIDE.md`, `opencode/_tracks-template/`, `docs/spec/` remain the repo guide,
  template scaffold, and requirements home.
- New-workstream seeding: `opencode/addBlazorFrontends` itself was seeded from
  `opencode/_tracks-template/` (see `AGENTIC-GUIDE.md` §4).
...
3. Seed a track from that repo's own template (or this repo's `opencode/_tracks-template/`):
