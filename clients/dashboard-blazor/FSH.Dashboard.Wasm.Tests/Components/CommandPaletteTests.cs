...
using FSH.Dashboard.Wasm.Shared;
...
public sealed class CommandPaletteTests : TestSetup
{
    ...
    {
        Authorize(FSH.BlazorShared.Permissions.CatalogPermissions.Products.View);
        RegisterTheme();
        var tree = RenderPalette(out var palette);
        await palette.Instance.OpenAsync();
        tree.WaitForAssertion(() => tree.Find(".fsh-command-input"));

        tree.Find(".fsh-command-input").Input("Products");

        tree.WaitForAssertion(() =>
        {
            tree.FindAll(".fsh-command-item").Count.ShouldBe(1);
            tree.Markup.ShouldContain("Products");
            tree.Markup.ShouldNotContain("Brands");
        });

        tree.Find(".fsh-command-item").Click();

        tree.WaitForAssertion(() =>
            Services.GetRequiredService<BunitNavigationManager>().Uri.ShouldEndWith("/catalog/products"));
    }

    [Fact]
    public async Task Permission_gated_destinations_are_hidden()
    {
        Authorize("Permissions.Catalog.Products.View");
        RegisterTheme();
        var tree = RenderPalette(out var palette);
        await palette.Instance.OpenAsync();
        tree.WaitForAssertion(() => tree.Find(".fsh-command-input"));

        tree.Find(".fsh-command-input").Input("Chat");

        tree.WaitForAssertion(() => tree.Markup.ShouldContain("No matches"));
        tree.Markup.ShouldNotContain(">Chat<");
    }
    ...
    public async Task Theme_actions_apply_and_keep_the_palette_open()
    {
        ...
        theme.Mode.ShouldBe(FSH.BlazorShared.Theming.ThemeMode.Dark);
        tree.WaitForAssertion(() => tree.Find(".fsh-command-input").ShouldNotBeNull());
    }
    ...
    {
        Authorize(
            FSH.BlazorShared.Permissions.CatalogPermissions.Products.View,
            FSH.BlazorShared.Permissions.CatalogPermissions.Brands.View,
            FSH.BlazorShared.Permissions.CatalogPermissions.Categories.View);
        RegisterTheme();
        var tree = RenderPalette(out var palette);
        await palette.Instance.OpenAsync();
        tree.WaitForAssertion(() => tree.Find(".fsh-command-input"));
        ...
        tree.WaitForAssertion(() =>
            Services.GetRequiredService<BunitNavigationManager>().Uri.ShouldEndWith("/catalog/brands"));
    }
