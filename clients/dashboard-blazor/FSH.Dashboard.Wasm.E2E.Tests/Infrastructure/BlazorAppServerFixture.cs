...
/// <summary>
/// Collection fixture for the dashboard-blazor E2E suite.
/// Owns the Playwright browser instance and the Blazor WASM dev server on port 5176
/// (mirrors the admin-blazor E2E fixture on 5175 and the React apps' Playwright
/// webServer: starts the app, reuses one that is already answering). The dev server
/// is started with the launch profile's URL via --urls, with the launch profile
/// disabled so no browser window is popped.
/// </summary>
public sealed class BlazorAppServerFixture : IAsyncLifetime, IDisposable
{
    public const int ServerPort = 5176;
    public const string ServerUrl = "http://localhost:5176";

    private static readonly string RepoRoot = FindRepoRoot();
    private static readonly string WasmProjectDir =
        Path.Combine(RepoRoot, "clients", "dashboard-blazor", "FSH.Dashboard.Wasm");
    ...
    private static string FindRepoRoot()
    {
        ...
        {
            if (Directory.Exists(Path.Combine(dir.FullName, "clients", "dashboard-blazor")))
            {
                return dir.FullName;
            }
    ...
    private async Task StartDevServerAsync()
    {
        ...
        psi.ArgumentList.Add(Path.Combine(WasmProjectDir, "FSH.Dashboard.Wasm.csproj"));
        ...
        _serverProcess = Process.Start(psi)
            ?? throw new InvalidOperationException("Failed to start the dashboard-blazor dev server.");

        var stdoutPath = Path.Combine(LogDir, "dashboard-devserver.stdout.log");
        var stderrPath = Path.Combine(LogDir, "dashboard-devserver.stderr.log");
