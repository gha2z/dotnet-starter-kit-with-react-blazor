...
- Phase 6: **🟢 Complete** — waves 1–15 committed (`e5bb0367`…`f7acfc16`). **All 6.x code items DONE** — **6.9 RESOLVED (wave 19 clean-publish re-test: wave-18 blocker was a stale-publish artifact; boot verified on plain static host)** and last-known-good cache (deferred) remain. See "Next Up".
...
- ⚠️ ~~**RELEASE-PUBLISH BLOCKER (6.9, re-tested wave 18 on SDK 10.0.302 — ROOT CAUSE SHIFTED)**: Release `dotnet publish` output **no longer boots at all**. **Wave-18 diagnosis**: SDK 10.0.302 publish emits the `_framework/*.js` files ONLY as `.br`/`.gz` compressed sidecars — the **uncompressed `blazor.webassembly.js` and `dotnet.js` are MISSING** from the output (native `.wasm` files still ship uncompressed). `index.html` references the uncompressed names → any plain static host 404s them, SPA fallback serves `index.html` (200, `text/html`) → browser throws `Unexpected token '<'` + `unsupported MIME type ('text/html')` → app never boots (`blazor-error-ui=false`, `mud-theme-provider=0`; verified via Playwright smoke). **`blazor.boot.json` is also completely absent** from `_framework/` (no `.json` at all) — the boot config is inlined into `dotnet.js` in .NET 10, which is itself missing uncompressed. The old **Mono NIY crash** (`LocalizationOptions:.ctor` NIY, interp.c:4135 — upstream dotnet/runtime #121849 family) is now **unreachable**: the WASM runtime never loads. Reproduced identically on admin-blazor publish (same missing-JS layout). Debug/DevServer pipeline (what E2E/bUnit exercise) remains fully green. **Workarounds**: (1) serve via ASP.NET static-file host / `dotnet serve` that content-negotiates `.br`/`.gz`; (2) pre-expand the compressed files in the publish output; (3) CDN/reverse-proxy must serve `.br`/`.gz` sidecars with `Content-Encoding` headers (also the 6.1 pre-compression deployment requirement). Re-test after SDK servicing.~~ **RE-TESTED wave 19 (clean publish): NOT REPRODUCIBLE — 6.9 RESOLVED.** Fresh clean Release publish (deleted `obj/Release` first) of BOTH apps emits the **uncompressed `blazor.webassembly.js` + `dotnet.js`** (plus `.br`/`.gz` sidecars); `blazor.boot.json` is absent by .NET 10 design (boot config inlined in `dotnet.js` between `/*json-start*/`/`/*json-end*/` — verified `mainAssemblyName`, `resources.assembly` (75 eager) + `lazyAssembly` correct). Boot smoke on plain static server with SPA fallback (port 5190): **login page fully renders in ~4.3 s, ZERO pageerrors, ZERO failed requests, no `#blazor-error-ui`** — and **no Mono NIY crash** (the old #121849 blocker). **Wave-18 conclusion was a stale-publish artifact**: reusing the `obj/Release` intermediates (prior configs' cached `wasm` intermediates + a partially-updated output folder) produced a publish output missing the uncompressed JS — the plan's own measurement gotcha ("always clean `obj/Release` between configs") applies to the publish output folder too. Rule: **for boot verification, always publish with a clean `obj/Release` (or fresh `-o` folder) and confirm `_framework/blazor.webassembly.js` + `dotnet.js` uncompressed exist before smoke-testing.**
...
- [x] ~~**6.9 RELEASE-PUBLISH BLOCKER**~~ **RESOLVED (wave 19 re-test, clean publish)**: wave-18 claim (SDK 10.0.302 emits only compressed `_framework/*.js`, no boot.json) was a **stale-publish artifact** — clean Release publish of both WASM apps emits uncompressed `blazor.webassembly.js` + `dotnet.js`; boot config inlined in dotnet.js (no boot.json = .NET 10 design); **plain static host boots to login in ~4.3 s with zero errors, no Mono NIY crash**. 6.9 CLOSED.
