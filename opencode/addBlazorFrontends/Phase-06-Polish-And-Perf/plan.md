...
- [ ] **6.9 RELEASE-PUBLISH BLOCKER** (re-tested wave 18, SDK 10.0.302): Release publish **missing uncompressed `_framework/*.js` + `blazor.boot.json`** → static hosting cannot boot (SPA fallback serves index.html as text/html, `Unexpected token '<'`). Old Mono NIY crash (`LocalizationOptions:.ctor`, interp.c:4135, upstream #121849) now unreachable — WASM never loads. Workaround: content-negotiation server / CDN serving `.br`/`.gz` with `Content-Encoding`. Re-test after SDK servicing or with ASP.NET static-file host.
